 ...
 *****************************************************************************/
#define MAX_RX_DATA_SIZE    6//256 for strings
 ...
 *************************************************************************/
		...
		size_t size_in_fifo;
		size_in_fifo = UART_fill_tx_fifo( &g_uart, (const uint8_t *)&g_message, sizeof(g_message) ); //prepare UART for message sending, send first heartbeat
		...
		//This group of IF statements checks for incoming commands and initiates motions if they are received, single ASCII characters are used to trigger subroutines for motion.
			...
			while(rx_data[0]){
				...
				if(rx_size > 0 && rx_data[0]== '*'){
							commandcontrol[0]=rx_data[0];
							delay(5);
							rx_size = UART_get_rx( &g_uart, rx_data, sizeof(rx_data) ); //Read UART buffer and get size
							commandcontrol[1]=rx_data[0];
							delay(5);
							rx_size = UART_get_rx( &g_uart, rx_data, sizeof(rx_data) ); //Read UART buffer and get size
							commandcontrol[2]=rx_data[0];
							delay(5);
							rx_size = UART_get_rx( &g_uart, rx_data, sizeof(rx_data) ); //Read UART buffer and get size
							commandcontrol[3]=rx_data[0];
							//for(x=0; x<4; x++){
							//	iprintf("%c", commandcontrol[x]);

							//}


#ifdef VERBOSEDEBUGCONSOLE
							iprintf("Command recieved\n"); //Identify operation via semihosting debug print (only check two not to overload semihosting)
#endif
							rx_size = UART_get_rx( &g_uart, rx_data, sizeof(rx_data) ); //Read UART buffer and get size
							//rx_data[0]=0;
							if(rx_data[0]== 'L'){
...
//								rx_data[0]= 0;
//							}
							if(rx_data[1]==76||rx_data[2]==76||rx_data[3]==76||rx_data[0]==76){
...
//										rx_data[0]=0;
#ifdef VERBOSEDEBUGCONSOLE
									iprintf("Routine '*L0O' Received - LED1\n"); //Identify operation via semihosting debug print (only check two not to overload semihosting)
#endif
										GPIO_set_output(&g_gpio, 0,  1);
									}else if(rx_data[0]=='F'){
//										rx_data[0]=0;
										GPIO_set_output(&g_gpio, 0, 0);
									}
#ifdef VERBOSEDEBUGCONSOLE
									iprintf("LED1 TURNEDON\n"); //Identify operation via semihosting debug print (only check two not to overload semihosting)
#endif
								}
								...
								if(rx_data[x+2]== '1'){
									rx_data[x+2]=0;
#ifdef VERBOSEDEBUGCONSOLE
									iprintf("Routine '*L1' Received - LED1\n"); //Identify operation via semihosting debug print (only check two not to overload semihosting)
#endif
									PWM_set_duty_cycle(&the_pwm, PWM_1, 50); //turn on LED2
									PWM_set_duty_cycle(&the_pwm, PWM_1, 0);
#ifdef VERBOSEDEBUGCONSOLE
									iprintf("LED2 TURNEDON\n"); //Identify operation via semihosting debug print (only check two not to overload semihosting)
#endif
								}

								if(rx_data[x+2]== '2'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_2, 50); //turn on LED3
									PWM_set_duty_cycle(&the_pwm, PWM_2, 0); //turn off
								}

								if(rx_data[x+2]== '3'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_3, 50); //turn on LED4
									PWM_set_duty_cycle(&the_pwm, PWM_3, 0); //turn off
								}

								if(rx_data[x+2]== '4'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_4, 50); //turn on LED5
									PWM_set_duty_cycle(&the_pwm, PWM_4, 0); //turn off
								}

								if(rx_data[x+2]== '5'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_5, 50); //turn on LED6
									PWM_set_duty_cycle(&the_pwm, PWM_5, 0); //turn off
								}

								if(rx_data[x+2]== '6'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_6, 50); //turn on LED7
									PWM_set_duty_cycle(&the_pwm, PWM_6, 0); //turn off
								}
								if(rx_data[x+2]== '7'){
									rx_data[x+2]=0;
									PWM_set_duty_cycle(&the_pwm, PWM_7, 50); //turn on LED8
