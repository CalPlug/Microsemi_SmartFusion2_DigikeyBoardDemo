...
The FTDI must be programmed so that Port B and Port C are presented as virtual com ports to the connected computer.  In programming, the lower com port is directly connected to the ESP32, the higher com port is connected via pin mapping in the SF2 to the ESP8266.  The Enable and reset functions are implemented in this firmware using the buttons on the demoboard.
To Program the ESP8266:
1) Plug in Device
2) Remove jumpers for J10 and J11 (this isolates the ESP32)
3)Press Button 2
4) Upload Code
5) Press Button 1 (resets ESP8266)
...
When a button is pressed, the D9 LED comes on and action is happening until LED extinguishes.  Proceed to the next step when LED is off.
